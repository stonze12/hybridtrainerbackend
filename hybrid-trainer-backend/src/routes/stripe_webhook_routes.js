...
const { PRO_MONTHLY_CREDITS } = require('../credits/pricing_config');
...
async function handleInvoicePaymentSucceeded(invoice) {
  ...
  try {
    ...
    await client.query(`UPDATE users SET membership_type = 'pro' WHERE id = $1`, [userId]);
  ...
  // Grant the monthly credits AFTER the subscription row is committed,
  // using the invoice id as the idempotency reference — if this exact
  // invoice already granted credits (shouldn't happen given the outer
  // webhook dedup, but defense in depth), grant_credits would still
  // insert a new ledger row; the outer dedup is what actually prevents
  // double-grants, this is just keeping the reference meaningful.
  await creditService.grantMonthlyProCredits({
    userId, subscriptionId: subscription.id, credits: PRO_MONTHLY_CREDITS,
  });
}
...
// Downgrade membership. Note: we do NOT claw back already-granted
// monthly credits — those were earned for the period already paid for.
// ----------------------------------------------------------------------------
