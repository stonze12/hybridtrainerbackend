...
async function registerUser({ email, username, password }) {
  ...
  try {
    ...
    // Signup bonus goes through the SAME atomic ledger function as every
    // other credit grant — no special-cased "set credits = 25" anywhere.
    await client.query(
      `SELECT * FROM grant_credits($1, $2, 'signup_bonus', NULL, $3)`,
      [user.id, 25, JSON.stringify({ reason: 'new_account' })]
    );
